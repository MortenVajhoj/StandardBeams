...
from .Standard import H_beam_standards , load_h_beam_sizes , load_h_beams
...
class Dialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("H-Beam")
        self.setMinimumWidth(550)
        self.setMinimumHeight(400)
        self.current_standard = list(H_beam_standards.keys())[0]

        self.load_current_standard()
        self.setup_ui()

    def load_current_standard(self):
        folder, beams_csv, sizes_csv = H_beam_standards[self.current_standard]
        self.h_beams = load_h_beams(folder, beams_csv)
        self.h_beam_sizes = load_h_beam_sizes(folder, sizes_csv)
    ...
    def setup_ui(self):
        ...
        self.standard_combo.addItems(list(H_beam_standards.keys()))
    ...
    def populate_sizes(self):
        self.size_table.setRowCount(len(self.h_beams))
        for row, size_data in enumerate(self.h_beams):
